...
inline void saveAllWorlds() // 
{
	...
	Sleep(200);
	GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w[`9IUTopia`w] `2Saved `oall worlds``"));
...
inline void sendWorld(ENetPeer* peer, WorldInfo* worldInfo)
{
...

	{
		...
		else if (worldInfo->items[i].foreground == 3818) {
...
inline void autoSaveWorlds() {
	while (1) {
		...
		GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w[`9IUTopia`w] `5Auto Saving All Worlds..."));
		ENetPacket* packet = enet_packet_create(p.data,
			p.len,
			ENET_PACKET_FLAG_RELIABLE);
		enet_host_broadcast(server, 0, packet);
